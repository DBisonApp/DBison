...
import { isValidBitcoinAddress, parseAuthor, supportOptions } from '../shared/package-meta.js'
...
describe('supportOptions', () => {
  it('treats the empty placeholders as nothing set up', () => {
    const pkg = { support: { kofi: '', crypto: [{ name: 'Bitcoin', network: 'Bitcoin', address: '' }] } }
    expect(supportOptions(pkg)).toEqual({ kofi: null, crypto: [] })
    expect(supportOptions({})).toEqual({ kofi: null, crypto: [] })
  })
